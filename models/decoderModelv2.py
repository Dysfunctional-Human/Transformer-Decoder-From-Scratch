...
class DecoderModelv2(nn.Module):
    def __init__(
        self, 
        vocab_size: int,
        EMBED_SIZE: int,
...
if __name__ == "__main__":
    ...
    demo_dir = temp_base / "decoderv2"
    
    # Saving model in the temporary demo directory
    save_model(
        model=decoder,
        model_name="sample_decoderv2_model.pt",
        target_dir=str(demo_dir),
    )

    loaded_model = load_model(
        model=decoder,
        target_model_path=str(demo_dir / "sample_decoderv2_model" / "sample_decoderv2_model.pt"),
    )
